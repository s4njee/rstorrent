...
//!
//! A port of `src-tauri/src/throttles.rs`. rtorrent stores a throttle name on
//! each download but forgets the name's rate definition when the daemon
//! restarts, so the definitions live in app settings and are replayed on each
//! connection. Slots actively assigned to a torrent are never repurposed; an
//! inactive stale slot can be recycled.

use std::collections::HashSet;

use crate::settings::NamedThrottle;

/// How many distinct per-torrent rate pairs the app will keep defined.
pub const POOL_SIZE: usize = 8;
const PREFIX: &str = "rstorrent_";

/// Choose a definition for a requested rate pair.
///
/// The boolean is true when the selected slot is new or must be redefined and
/// persisted, and false when an existing definition already matches.
///
/// # Errors
///
/// Returns a message when every slot is both defined and in active use.
pub fn allocate(

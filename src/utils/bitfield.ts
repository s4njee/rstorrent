...
/**
 * Decode rtorrent's `d.chunks_seen` into per-chunk peer counts — one byte per
 * chunk (0..255, rtorrent saturates), the same hex-pair wire format as
 * `bitfieldToBytes` but with a different interpretation: the count of connected
 * peers holding each chunk rather than a bitmask. Bad/empty input yields an
 * empty array.
 */
export function availabilityToBytes(hex: string): Uint8Array {
  return bitfieldToBytes(hex);
}
 ...
 *
 * (rtorrent's own `distributed_copies()` adds our have-bit into the count and
 * handles all-set bitfields; this approximation intentionally stays simpler.)
 */
